	...
	useEffect(() => {
		...
		newCanvas.on("object:moving", (e) => {
			...
			if (obj && obj.type === "image") {
				...
				if (obj.left + obj.getScaledWidth() > newCanvas.width)
					obj.left = newCanvas.width - obj.getScaledWidth();
				if (obj.top + obj.getScaledHeight() > newCanvas.height)
					obj.top = newCanvas.height - obj.getScaledHeight();
			}
		});
	...
	useEffect(() => {
		...
		if (contentElement && popUpElement) {
			...
			if (isPopupVisible) {
				...
				popUpElement
					.querySelectorAll(".info, .extension, .export")
					.forEach((element) => {
